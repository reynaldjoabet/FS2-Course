...
def readLegoSetImperative(filename:String,predicate:LegoSet=>Boolean,limit:Int):List[LegoSet]={
...
def readLegoSetList(filename:String,predicate:LegoSet=>Boolean,limit:Int):List[LegoSet]={
    JFiles.readAllLines(Paths.get(filename))
    .asScala.flatMap(parseLegoSet)
    .filter(predicate)
    .take(limit)
    .toList
...
def readLegoSetIterator(filename:String,predicate:LegoSet=>Boolean,limit:Int):List[LegoSet]={
...
def readLegoSetStreams(filename:String,predicate:LegoSet=>Boolean,limit:Int):IO[List[LegoSet]]={
...
.toList
    ...
    override def run: IO[Unit] =readLegoSetStreams("sets.csv",_.year > 1980,10)
